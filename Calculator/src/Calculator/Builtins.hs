...
import Calculator.Types (Assoc (..), ExecFn (..), ExecOp (..), Expr (..), Fun (..), FunFun (..), FunMap, FunOp (..), Op (..), OpMap, VarMap)
...
functions =
  ...
  , (("sinc", 1), Fun{params = ["x"], fexec = ExFn (Call "if" [Call "!=" [Id "x", Number 0 0], Call "/" [Call "sin" [Id "x"], Id "x"], Number 1 0])})
